...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        x, y = cell
        first_row = max(0, x-1)
        sec_row = min(self.height, x+2)
        first_col = max(0, y-1)
        sec_col = min(self.width, y+2)
        for i in range(first_row, sec_row):
            for j in range(first_col, sec_col):
        ...
        inferred_sentences = self.find_subset(new_sentence, self.knowledge)
    ...
    def find_subset(self, sentence, sentences):
        new_sentences = []
        for each in sentences:
            is_subset = True
            for cell in sentence.cells:
                if cell not in each.cells:
                    is_subset = False
            if is_subset:
                new_sentence_set = set()
                for cell in each.cells:
                    if cell not in sentence.cells:
                        new_sentence_set.add(cell)
                new_sentence_count = each.count - sentence.count
                new_sentences.append(Sentence(new_sentence_set, new_sentence_count))
